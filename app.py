...
@auth.login_required
def api_loans_debts_alias():
    """Alias for /api/loans/my-debts (the mobile loans.ts calls this name)."""
    return api_loans_my_debts()

@app.route("/api/loans/credits")
@auth.login_required
def api_loans_credits_alias():
    """Alias for /api/loans/my-credits (the mobile loans.ts calls this name)."""
    return api_loans_my_credits()
...
# ── ROUTE: POST /api/pool/create ─NEW Mobile Additions────────────────────────────────────────────
...
def api_pool_create():
    ...
    Required: name, target_cents, currency
    Optional: description, deadline, reminder_frequency_days, max_members, is_public
    """
    uid = session["user_id"]
    d = request.json or {}
    ...
    target_cents = int(d.get("target_cents", 0))
    if target_cents < 1000:  # minimum €10
        return jsonify({"error": "Target must be at least €10"}), 400
 
    currency = d.get("currency", "EUR")
    description = (d.get("description") or "").strip()
    deadline = d.get("deadline") or None
    reminder_freq = int(d.get("reminder_frequency_days", 7))
    max_members = int(d.get("max_members", 20))
    is_public = 1 if d.get("is_public", True) else 0
    ...
    with get_db(immediate=True) as db:
        # Create the pool
        db.execute("""INSERT INTO pools
            (id, name, slug, description, creator_id, currency, target_cents,
             deadline, reminder_frequency_days, max_members, member_count,
             status, is_public, created_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'))""",
            (pool_id, name, slug, description, uid, currency, target_cents,
             deadline, reminder_freq, max_members, 1,  # creator counts as member
             "active", is_public))
 
        # Add creator as first member with 'admin' role
        db.execute("""INSERT INTO pool_members (id, pool_id, user_id, role, status)
            VALUES (?,?,?,?,?)""",
            (str(uuid.uuid4()), pool_id, uid, "admin", "active"))
 
    return jsonify({"ok": True, "pool_id": pool_id, "name": name}), 201
 
 
# ── ROUTE: POST /api/pool/<id>/join ──────────────────────────────────────────
 
@app.route("/api/pool/<pool_id>/join", methods=["POST"])
@auth.login_required
def api_pool_join(pool_id):
    ...
    existing = fetchone("SELECT id FROM pool_members WHERE pool_id=? AND user_id=?", (pool_id, uid))
    if existing:
        return jsonify({"error": "You're already a member of this pool"}), 400
 
    # Check capacity
    count = fetchone("SELECT COUNT(*) as c FROM pool_members WHERE pool_id=? AND status='active'", (pool_id,))
    if count and count["c"] >= pool["max_members"]:
        return jsonify({"error": "This pool is full"}), 400
 
    status = "active"  # pools auto-approve by default
 
    with get_db(immediate=True) as db:
        db.execute("INSERT INTO pool_members (id, pool_id, user_id, role, status) VALUES (?,?,?,?,?)",
                   (str(uuid.uuid4()), pool_id, uid, "member", status))
        db.execute("UPDATE pools SET member_count = member_count + 1 WHERE id=?", (pool_id,))
 
    return jsonify({"ok": True, "status": status})
 
 
# ── ROUTE: POST /api/pool/<id>/contribute ────────────────────────────────────
...
@app.route("/api/pool/<pool_id>/approve-member", methods=["POST"])
@auth.login_required
def api_pool_approve_member(pool_id):
    """Approve a pending member (organiser only)."""
    uid = session["user_id"]
    d = request.json or {}
    ...
    with get_db(immediate=True) as db:
        db.execute("UPDATE pool_members SET status='active' WHERE pool_id=? AND user_id=?",
                   (pool_id, target_uid))
        db.execute("UPDATE pools SET member_count = (SELECT COUNT(*) FROM pool_members WHERE pool_id=? AND status='active') WHERE id=?",
                   (pool_id, pool_id))
 
    return jsonify({"ok": True})
 
 
# ── ROUTE: POST /api/pool/<id>/remove-member ─────────────────────────────────
 
@app.route("/api/pool/<pool_id>/remove-member", methods=["POST"])
@auth.login_required
def api_pool_remove_member(pool_id):
    """Remove a member (organiser only)."""
    uid = session["user_id"]
    d = request.json or {}
    target_uid = d.get("user_id", "")
    ...
    with get_db(immediate=True) as db:
        db.execute("DELETE FROM pool_members WHERE pool_id=? AND user_id=?", (pool_id, target_uid))
        db.execute("UPDATE pools SET member_count = (SELECT COUNT(*) FROM pool_members WHERE pool_id=? AND status='active') WHERE id=?",
                   (pool_id, pool_id))
